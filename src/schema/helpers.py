...
from src.schemas.response import TimelineResponse, WorkflowCompleteResponse
...
def format_workflow_response(result: dict[str, Any]) -> WorkflowCompleteResponse:
    """Format the raw workflow state into a structured API response."""
    status = "success" if not result.get("error") else "failed"
    ...
    if result.get("timetable"):
        timeline = TimelineResponse(
            milestones=result.get("milestones", []),
            parallel_work_streams=result.get("parallel_work_streams", []),
            gantt_chart=result.get("timetable") if "gantt" in result.get("timetable", "").lower() else None,
            raw_timetable=result["timetable"],
        )
    ...
    return WorkflowCompleteResponse(
        status=status,
        proposal_summary=result.get("proposal_summary", result.get("proposal", "")[:200]),
        optimized_prompt=result.get("optimized_prompt"),
